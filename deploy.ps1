...
if (Test-Path $GhExe) { $env:Path = "$(Split-Path $GhExe);$env:Path" }
...
if (-not (Get-Command git -ErrorAction SilentlyContinue)) {
    Write-Host ""
    Write-Host "Git is not installed. Install it, then run this script again:"
    Write-Host "  winget install Git.Git"
    Write-Host ""
    Write-Host "After install, close this window and run DEPLOY-NOW.bat again."
    exit 1
}
